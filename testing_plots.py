...
# FIND SNPs BY COUNTY
# ---------------------------------------------------------------------------
def find_SNPs_by_county_2(input_fname):
...
def find_SNPs_by_county(input_fname): 
    ...
    for record in ny_aligned:
        ...
        if county not in records_by_county:
            ...
            records_by_county[county].append(record)
    print(records_by_county.keys())
    print(len(records_by_county.values()))
    for county in records_by_county:
        print(len(records_by_county[county]))
    
    for county in records_by_county:
        records = records_by_county[county]
        n = len(records)
        seg_sites = find_seg_sites(records)
        sites_for_county = []
        variants_for_county = []
        snp_cts_for_county = np.zeros(len(sites))
        print('List of SNPs for:', county) 
        snps = [] 
        for ss in seg_sites: 
            to_print = True 
            for base in ss[1]: 
            ...
            if to_print: 
                #print('locus: ', ss[0], ss[1]) 
                snps.append(ss) 
                sites_for_county.append(ss[0])
                variants_for_county.append(ss[1])
        for i in range(len(sites)):
            if sites[i] in sites_for_county:
                derived_at_site = derived_alleles[i]
                idx = sites_for_county.index(sites[i])
                snp_cts_for_county[i] = (variants_for_county[idx][derived_at_site])/n
            else:
                snp_cts_for_county[i] = 0
        #print(snps)
        snp_counts[county] = snp_cts_for_county
    print('find_SNPs_by_county done.') 
    return(snp_counts)

#snps = find_SNPs('final_ny_aligned.txt')

snp_counts, nums = find_SNPs_by_county_2('final_ny_aligned.txt')
print(snp_counts)
...
for county in snp_counts:   
    plt.bar(sites, snp_counts[county]/nums[county])
